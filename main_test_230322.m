...
for iter = 1:Niter
  GAM2 = diag(L*gamma2);
  J = ( G'*SIGMAi*G + I_A*GAM2 ) \ G'*SIGMAi*Y;
  ...
  Q = Q/T;
  %SIGMAi = inv(cov((G*J-Y)'));
  SIGMAi = inv(Q);
  IA_J = I_A*J;
  %for k = 1:K
  %  gamma2(k) = min( gamma2(k), ...
  %    1/( ( norm(IA_J(R{k},:),'fro')^2 )/(Nk(k)*T) ) );
  %end
  for k = 1:K
    gamma2(k) = 1/( ( norm(IA_J(R{k},:),'fro')^2 )/(Nk(k)*T) ) ;
  end
  
  %gamma2 = gamma2/sigma2;
  ERR(iter) = norm(G*J,'fro')/(N*T);
...
plot(time,synthetics_data_01.F)
